...
class TokensModel extends ListModel
{
	public function __construct($config = [])
	{
		if (empty($config['filter_fields'])) {
			$config['filter_fields'] = [
				...
				'last_used_at', 'a.last_used_at', 'created', 'a.created',
			];
		}
	...
	protected function getListQuery()
	{
		...
		$query = $db->getQuery(true)
			->select($db->quoteName(['a.id', 'a.item_id', 'a.label', 'a.customer_email', 'a.token_prefix', 'a.state', 'a.expires_at', 'a.max_uses', 'a.used_count', 'a.last_used_at', 'a.created']))
